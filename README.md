...
```
This would use `Sample-Input/example.tsv` for certain parametric information, [as described in the next section](#input). Note that if no [additional arguments](#options) are given, the program has certain default behaviors (read about this in the [options](#command-line-arguments) section further below).
### Input
